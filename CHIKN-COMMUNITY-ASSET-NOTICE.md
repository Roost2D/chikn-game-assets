# Chikn Community Asset Notice

The Chikn, Roostr and FarmLand visual and audio assets classified in
`manifests/rights-manifest.json` remain owned by Chikn. Chikn grants the
applicable non-commercial rights directly to Community Users under the
unmodified `CHIKN-COMMUNITY-ASSET-LICENSE_PUBLIC.md`, Version 1.1.
The repository-level boundary is recorded in the unmodified
`REPOSITORY-LICENSING-NOTICE_PUBLIC.md`.

Roost2D is an authorised distributor only. It does not own or sublicense
Chikn intellectual property and grants no commercial rights.

Required attribution:

> Chikn™, chikn™, Roostr™ and FarmLand™ assets © Chikn. Used under the Chikn Community Asset Pack Non-Commercial Licence. Commercial licensing: chikn.farm.

Commercial use requires a separate written agreement with Chikn.

The manifest contains per-file hashes only for technical integrity and
source-to-runtime traceability.

Files listed under `excludedPaths` are outside the Approved Asset Pack and
receive no rights from this notice or licence.

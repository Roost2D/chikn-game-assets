...
test('pack loader indexes canonical ids and aliases', async () => {
  ...
  const pack = await loadChiknPack({ manifestUrl: 'https://cdn.example/manifest.json', fetch: async () => new Response(JSON.stringify(manifest)) }); assert.equal(pack.findAsset('a').id, 'chikn/a');
});
